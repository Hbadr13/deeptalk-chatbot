        ...
        (
            async () => {
                try {
                    ...
                    if (session) {
                        const res = await fetch(`http://localhost:3000/api/messages/reaction/favorite`, {
                            headers: {
...

                {
                    messages.map((message, index) => (
                        ...
                        <div key={index} className="whitespace-pre-wrap  flex justify-end  relative">
                            <div className={`${message.role == 'user' ? 'bg-[#fce8e7]' : 'bg-[#D5DAE7]'} w-full  p-6 rounded-lg space-y-4 `} >
